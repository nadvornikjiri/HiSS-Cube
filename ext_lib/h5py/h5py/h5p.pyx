...
cdef class PropFCID(PropOCID):
    ...
    if HDF5_VERSION >= (1, 10, 1):
        ...
        def set_sym_k(self, ik, lk):
            ...
            """
            return H5Pset_sym_k(self.id, ik, lk)

        @with_phil
        def get_sym_k(self):
            """ () => TUPLE(UINT ik, UINT lk)

            Retrieves the size of the symbol table B-tree 1/2 rank and the symbol table leaf node 1/2 size.

            Returns
            -------
            Returns a tuple of the ik and lk - Symbol table's B-tree 1/2 rank and Symbol table's leaf node 1/2 size.
            """
