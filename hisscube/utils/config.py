...
class Config:
    def __init__(self, config_path="config/config.ini"):
        ...
        if self.IOC_SELECTION == "one_per_node":
            os.environ["H5FD_SUBFILING_IOC_PER_NODE"] = self.config.get('Subfiling', 'IOC_PER_NODE')
        if self.IOC_SELECTION == "every_nth_rank" or self.IOC_SELECTION == "total":
            os.environ["H5FD_SUBFILING_IOC_SELECTION_CRITERIA "] = self.config.get('Subfiling',
                                                                                   'IOC_SELECTION_CRITERIA')
