...
from hisscube.dependency_injector import SerialBuilderProvider, ParallelBuilderProvider
...
from hisscube.processors.metadata_image import ImageMetadataProcessor, get_image_header_dataset
from hisscube.processors.metadata_spectrum import SpectrumMetadataProcessor, get_spectrum_header_dataset
from hisscube.utils.config import Config
from hisscube.utils.io import truncate, H5Connector, CBoostedMetadataBuildConnector
...
class HiSSCubeConstructionDirector:
    def __init__(self, cli_args, config: Config, serial_builders: SerialBuilderProvider,
                 parallel_builders: ParallelBuilderProvider):
        self.args = cli_args
        self.config = config
...
class MetadataCacheBuilder(Builder):
    def __init__(self, fits_image_path: string, fits_spectra_path: string, config: Config,
                 h5_connector: H5Connector, metadata_processor: MetadataProcessor):
...
class CBoosterMetadataBuilder(MetadataBuilder):
    def build(self):
        with self.h5_connector as h5_connector:
            ...
            self.spectrum_metadata_processor.write_spectra_metadata(h5_connector)
            self.c_write_hdf5_metadata(self.h5_connector)

    def c_write_hdf5_metadata(self, h5_connector: CBoostedMetadataBuildConnector):
...
class SingleImageBuilder(Builder):
    ...
    def build(self):
        ...
        with self.h5_connector as h5_connector:
            self.build_image(h5_connector, self.image_path, self.image_metadata_processor, self.image_data_processor)
    ...
    def build_data(self, h5_connector, image_path, metadata_processor, data_processor, fits_file_name):
        metadata_processor.metadata, data_processor.data = self.photometry.get_multiple_resolution_image(image_path,
                                                                                                         self.config.IMG_ZOOM_CNT)
        res_grp_list = metadata_processor.get_resolution_groups(h5_connector)
...
class SingleSpectrumBuilder(Builder):
    ...
    def build(self):
        ...
        with self.h5_connector as h5_connector:
            self.build_spectrum(h5_connector, self.spectrum_path, self.spectrum_metadata_processor,
                                self.spectrum_data_processor)

    def build_spectrum(self, h5_connector, spec_path, metadata_processor, data_processor):
        """
        ...
        fits_folder_path = path.parent
        fits_file_name = path.name
        metadata_processor.update_spectra_headers(h5_connector, fits_folder_path, spec_pattern=fits_file_name)
        fits_header = get_spectrum_header_dataset(h5_connector)[metadata_processor.spec_cnt - 1][
            1]  # this is the JSON-like FITS header
        metadata_processor.write_spectrum_metadata(h5_connector, spec_path, fits_header)
        spec_datasets = self.build_data(h5_connector, spec_path, metadata_processor, data_processor, fits_file_name)
        return spec_datasets

    def build_data(self, h5_connector, spec_path, metadata_processor, data_processor, fits_file_name):
        metadata_processor.metadata, data_processor.data = self.photometry.get_multiple_resolution_spectrum(
...
class DataBuilder(Builder):
    ...
    def build(self):
        with self.h5_connector as h5_connector:
            image_paths = get_image_header_dataset(h5_connector)[0][:]
            spec_paths = get_spectrum_header_dataset(h5_connector)[0][:]
            for image_path in tqdm(image_paths, desc="Image Progress: "):
            ...
            for spec_path in tqdm(spec_paths, desc="Spectra Progress: "):
                fits_file_name = Path(image_path).name
...
class ParallelDataBuilder(Builder, metaclass=ABCMeta):
    ...
    def build(self):
        with self.h5_connector as h5_connector:
            if self.mpi_helper.mpi_rank == 0:
                self.distribute_work(h5_connector, get_image_header_dataset(h5_connector)[0][:],
            ...
            if self.mpi_helper.mpi_rank == 0:
                self.distribute_work(h5_connector, get_spectrum_header_dataset(h5_connector)[0][:],
                                     self.spectrum_metadata_processor, self.spectrum_data_processor)
            else:
    ...
    def distribute_work(self, h5_connector, path_list, metadata_processor, data_processor):
        ...
        for i in tqdm(range(1, len(batches) + 1)):
            if i < self.mpi_helper.mpi_size:
                self.mpi_helper.send_work(batches, dest=i)
                self.mpi_helper.active_workers += 1
            else:
                self.mpi_helper.wait_for_message(source=MPI.ANY_SOURCE, tag=self.mpi_helper.FINISHED_TAG, status=status)
        ...
        for i in range(1, self.mpi_helper.mpi_size):
...
class ParallelMWMRDataBuilder(ParallelDataBuilder):
    ...
    def process_image_data(self, h5_connector):
        ...
        while status.Get_tag() != self.mpi_helper.KILL_TAG:
            for image_path in image_path_list:
                try:
                    self.logger.debug("Rank %02d: Processing image %s." % (self.mpi_helper.mpi_rank, image_path))
    ...
    def process_spectra_data(self, h5_connector):
        ...
        while status.Get_tag() != self.mpi_helper.KILL_TAG:
            for spec_path in spectra_path_list:
                try:
                    self.logger.debug("Rank %02d: Processing spectrum %s." % (self.mpi_helper.mpi_rank, spec_path))
    ...
    def process_response(self, h5_connector, metadata_processor, data_processor, status):
        self.mpi_helper.comm.recv(source=MPI.ANY_SOURCE, tag=self.mpi_helper.FINISHED_TAG, status=status)
        self.metadata_processor.fits_total_cnt += 1
        self.logger.debug(
...
class ParallelSWMRDataBuilder(ParallelDataBuilder):
    ...
    def process_image_data(self, h5_connector):
        ...
        while status.Get_tag() != self.mpi_helper.KILL_TAG:
            ...
            for image_path in image_path_list:
                self.logger.info("Rank %02d: Processing image %s." % (self.mpi_helper.mpi_rank, image_path))
    ...
    def process_spectra_data(self, h5_connector):
        ...
        while status.Get_tag() != self.mpi_helper.KILL_TAG:
            ...
            for spec_path in spectra_path_list:
                self.logger.info("Rank %02d: Processing spectrum %s." % (self.mpi_helper.mpi_rank, spec_path))
                self.spectrum_metadata_processor.metadata, self.spectrum_data_processor.data = self.photometry.get_multiple_resolution_spectrum(
                    spec_path, self.config.SPEC_ZOOM_CNT,
                    apply_rebin=self.config.APPLY_REBIN,
                    rebin_min=self.config.REBIN_MIN,
    ...
    def add_to_result_batch(fits_path, processed_image_batch, metadata_processor, data_processor):
        ...
        processed_image_batch.append(
            {"metadata": dict(metadata_processor.metadata), "data": data_processor.data, "file_name": str(file_name)})
...
class LinkBuilder(Builder):
    ...
    def build(self):
        with self.h5_connector as h5_connector:
            self.spectrum_metadata_processor.add_image_refs(h5_connector.f)


class MLCubeBuilder(Builder):
...
class VisualizationCubeBuilder(Builder):
    def __init__(self, config: Config, h5_connector: H5Connector,
