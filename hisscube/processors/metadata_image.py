...
class ImageMetadataProcessor:
    ...
    def update_image_headers(self, h5_connector, image_path, image_pattern=None):
        ...
        try:
            self.img_cnt = self.h5_connector.f.attrs["image_count"]  # header datasets not created yet
        except KeyError:
            self.img_cnt = 0
            self.metadata_handler.create_fits_header_datasets()
        image_header_ds = get_image_header_dataset(h5_connector)
        self.img_cnt += self.metadata_handler.write_fits_headers(image_header_ds, image_header_ds.dtype, image_path,
                                                                 image_pattern,
                                                                 self.config.LIMIT_IMAGE_COUNT, offset=self.img_cnt)
        self.h5_connector.f.attrs["image_count"] = self.img_cnt
    ...
    def require_image_spatial_grp_structure(self, parent_grp):
        ...
        for order in range(self.config.IMG_SPAT_INDEX_ORDER):
            parent_grp = self.metadata_handler.require_spatial_grp(order, parent_grp, image_coords)
    ...
    def require_image_spectral_grp(self, parent_grp):
        grp = self.h5_connector.require_group(parent_grp, str(
            self.metadata_handler.photometry.filter_midpoints[self.metadata["FILTER"]]),
                                              track_order=True)
    ...
    def write_images_metadata(self, h5_connector, no_attrs=False, no_datasets=False):
        self.h5_connector = h5_connector
        fits_headers = self.h5_connector.f["/fits_images_metadata"]
    ...
    def write_image_metadata_from_cache(self, h5_connector, fits_headers, no_attrs, no_datasets):
        ...
        for fits_path, header in fits_headers:
            ...
            if self.img_cnt >= self.config.LIMIT_IMAGE_COUNT:
                break
        self.h5_connector.set_attr(self.h5_connector.f, "image_count", self.img_cnt)
    ...
    def write_parsed_image_metadata(self, fits_path, no_attrs, no_datasets):
        ...
        if not no_attrs:
            self.metadata_handler.add_metadata(img_datasets)
    ...
    def get_resolution_groups(self, h5_connector):
        ...
        tai_time = self.metadata["TAI"]
        spectral_midpoint = self.metadata_handler.photometry.filter_midpoints[self.metadata["FILTER"]]
        path = "/".join([spatial_path, str(tai_time), str(spectral_midpoint)])
        spectral_grp = self.h5_connector.f[path]
    ...
    def create_img_datasets(self, parent_grp_list):
        ...
        for group in parent_grp_list:
                ...
                if "image_dataset" in group:
                    raise ValueError(
                        "There is already an image dataset %s within this resolution group. Trying to insert image %s." % (
                            list(group["image_dataset"]), self.metadata_handler.file_name))
            elif len(group) > 0:
                raise ValueError(
                    "There is already an image dataset %s within this resolution group. Trying to insert image %s." % (
            ...
            ds = self.h5_connector.create_image_h5_dataset(group, self.metadata_handler.file_name, img_data_shape)
